...
export function FormularioCliente({ cliente }: { cliente?: ClienteEditavel }) {
  return (
    <div className="space-y-4">
      <Card>
        <form action={salvarCliente} className="space-y-8">
          {cliente && <input type="hidden" name="id" value={cliente.id} />}

          <FormSection
            title="Identificação"
            subtitle="Como o cliente aparece no sistema."
          >
          ...
          <FormSection
            title="Contato"
            subtitle="Com quem falar no dia a dia."
          >
            <div className="grid gap-4 sm:grid-cols-2">
          ...
          <FormSection
            title="Relação com a agência"
            subtitle="Desde quando é cliente, quanto paga e como está a relação."
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <Field
                label="Cliente desde"
                hint="Usado para calcular o tempo de casa."
              >
                <Input
                  name="startDate"
                  type="date"
              ...
              <Field label="Situação">
                <Select
                  ...
                  options={Object.entries(CLIENT_STATUS).map(
              ...
              <Field
                label="Quanto paga por mês (R$)"
                hint="Valor combinado com o cliente. Deixe em branco se preferir controlar só pelos contratos."
              >
                <Input
                  name="monthlyFee"
                  type="number"
                  step="0.01"
                  min={0}
                  defaultValue={cliente?.monthlyFee ?? ""}
                  placeholder="0,00"
                  inputMode="decimal"
                />
              </Field>
          ...
          <FormSection
            title="Observações"
            subtitle="O que a equipe precisa lembrar sobre esse cliente."
          >
            <Field label="Notas">
              <Textarea
                name="notes"
                ...
                placeholder="Preferências, combinados, tom de voz, contatos secundários…"
              />
            </Field>
          </FormSection>
